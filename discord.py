...
class DiscordWebhook:
    ...
    async def send_clip_notification(
        self,
        clip_url: str,
        comments: str = "",
        ...
        # Create embed fields
        fields = [
            {"name": "Broadcaster", "value": f"`{broadcaster_name}`", "inline": False},
            {"name": "Clip Title", "value": f"`{clip_title}`", "inline": False},
            {"name": "Directory", "value": f"`{directory}`", "inline": False},
            {
                "name": "Voted By",
        ...
        if comments:
            fields.append({"name": "Comments", "value": comments, "inline": False})

        # Add URL and Direct Download
        fields.extend([{"name": "URL", "value": clip_url, "inline": False}])
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.webhook_url, json=payload) as response:
                    if response.status == 204:
                        ...
                        log.error(
                            f"Failed to send Discord webhook: {response.status} - {error_text}"
                        )
